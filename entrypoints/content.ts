import type { BackgroundToContentMessage, ContentToBackgroundMessage, VoiceboxState } from '../lib/types';
...
const generateId = (() => {
  let id = 0;
  const generator = (prefix: string) => `_voicebox_${Date.now()}-${prefix}-${id++}`;
  return <T extends string>(...ids: T[]) =>
    Object.fromEntries(ids.map((id) => [id, generator(id)] as const)) as Record<T, string>;
})();
...
function drawWaveform(analyserNode: AnalyserNode, canvas: HTMLCanvasElement, isProcessing: boolean = false) {
  ...
  const draw = () => {
    if (currentState !== 'listening' && currentState !== 'speaking' && !isProcessing) return;
...
function updateState(newState: VoiceboxState) {
  ...
  switch (newState) {
    case 'idle':
      ...
      break;
    case 'listening':
      ui.statusText.textContent = 'Listening... (Click mic or press Alt+X to stop)';
