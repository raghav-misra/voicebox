...
async function normalizeCoordinates(tabId: number, { x, y }: Pair): Promise<Pair> {
  const { cssLayoutViewport } = await sendCommand<Protocol.Page.GetLayoutMetricsResponse>(
    tabId,
...
const keyDefinitions: Record<string, { key: string; code: string; vk: number }> = {
...
export const browserAgentActions = {
  ...
  async RELOAD_TAB(tabId: number, { options }: { options?: { ignoreCache?: boolean } }) {
    try {
      await sendCommand(tabId, 'Page.reload', { ignoreCache: options?.ignoreCache });
  ...
  async GO_BACK(tabId: number) {
    try {
      const { entries, currentIndex } = await sendCommand<Protocol.Page.GetNavigationHistoryResponse>(
        tabId,
        'Page.getNavigationHistory'
      );
      const prev = entries[currentIndex - 1];
      if (prev) {
        await sendCommand(tabId, 'Page.navigateToHistoryEntry', { entryId: prev.id });
      }
  ...
  async GO_FORWARD(tabId: number) {
    try {
      ...
      if (next) {
        await sendCommand(tabId, 'Page.navigateToHistoryEntry', { entryId: next.id });
      }
  ...
  async CLICK(
    tabId: number,
    { x, y, options }: { x: number; y: number; options?: { button?: 'left' | 'right' | 'middle'; clickCount?: number } }
  ) {
  ...
  async DOUBLE_CLICK(tabId: number, { x, y }: { x: number; y: number }) {
    return browserAgentActions.CLICK(tabId, { x, y, options: { clickCount: 2, button: 'left' } });
  },

  async SCROLL(tabId: number, { x, y, deltaX, deltaY }: { x: number; y: number; deltaX: number; deltaY: number }) {
    const coords = await normalizeCoordinates(tabId, { x, y });
    const deltaCoords = await normalizeCoordinates(tabId, { x: deltaX, y: deltaY });
  ...
  async SCROLL_DOCUMENT(tabId: number, { direction, magnitude }: { direction: 'up' | 'down' | 'left' | 'right'; magnitude: number }) {
    let deltaX = 0;
    let deltaY = 0;
    ...
    const normalized = await normalizeCoordinates(tabId, { x: deltaX, y: deltaY });
  ...
  async DRAG_AND_DROP(
    tabId: number,
    {
      ...
      toX: number;
      toY: number;
      options?: { button?: 'left' | 'right' | 'middle'; steps?: number; delay?: number };
    }
  ) {
    const button = options?.button ?? 'left';
    ...
    const fromCoords = await normalizeCoordinates(tabId, { x: fromX, y: fromY });
  ...
  async TYPE_TEXT(tabId: number, { text, options }: { text: string; options?: { delay?: number } }) {
    const delay = Math.max(0, options?.delay ?? 0);

    try {
      const keyStroke = async (
        ch: string,
        override?: { key?: string; code?: string; windowsVirtualKeyCode?: number }
      ) => {
        if (override) {
          ...
          await sendCommand(tabId, 'Input.dispatchKeyEvent', { ...base, type: 'keyUp' } as Protocol.Input.DispatchKeyEventRequest);
          return;
        }
        ...
        await sendCommand(tabId, 'Input.dispatchKeyEvent', { type: 'keyUp' } as Protocol.Input.DispatchKeyEventRequest);
      };

      for (const ch of text) {
        if (ch === '\n' || ch === '\r') {
          await keyStroke(ch, { key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13 });
        } else if (ch === '\t') {
          await keyStroke(ch, { key: 'Tab', code: 'Tab', windowsVirtualKeyCode: 9 });
        } else {
          await keyStroke(ch);
        }
        if (delay) await waitMs(delay);
  ...
  async KEY_PRESS(tabId: number, { key, options }: { key: string; options?: { delay?: number } }) {
    const delay = Math.max(0, options?.delay ?? 0);
    ...
    try {
      ...
      const type = hasNonShiftModifier || mainKey.length > 1 ? 'rawKeyDown' : 'keyDown';
      ...
      if (type === 'keyDown') {
        keyDownParams.text = modifiers & modifierMap.Shift ? mainKey.toUpperCase() : mainKey.toLowerCase();
  ...
  async CAPTURE_SCREENSHOT(tabId: number) {
    await waitMs(500);
    const { data } = await sendCommand<Protocol.Page.CaptureScreenshotResponse>(tabId, 'Page.captureScreenshot', {
      format: 'png',
    });
 ...
 */
export async function executeAction(tabId: number, action: BrowserAgentAction): Promise<any> {
  const handler = browserAgentActions[action.type as keyof typeof browserAgentActions];
  if (!handler) {
